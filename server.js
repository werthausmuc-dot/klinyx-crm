...
server.listen(PORT, () => {
  ...
  console.log("Storage backend: " + (process.env.MONGODB_URI ? "MongoDB (" + (process.env.MONGODB_DB || "klinyx_crm") + ")" : "local file (data/db.json)"));
  Promise.resolve(store.hasAnyUsers())
    .then((has) => {
      console.log(has ? "Ready — sign in at /." : "No users yet — open the app in a browser to create the first admin account.");
    })
    .catch((err) => {
